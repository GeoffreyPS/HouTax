defmodule DeltaFunsTest do
  ...
  test "function find_annual_deltas returns a Building with Annual Deltas for each year", meta do
    ...
    %{2012 => %TaxValue{annual_delta: ad_2012}, 
      ...
      2014 => %TaxValue{annual_delta: ad_2014}} = Map.get(Building.DeltaFuns.find_annual_deltas(meta[:building]), :tax_values)
  ...
  test "function find_value_deltas returns a Building with Value Deltas for each year", meta do
    ...
    %{2012 => %TaxValue{value_delta: vd_2012}, 
      ...
      2014 => %TaxValue{value_delta: vd_2014}} = Map.get(Building.DeltaFuns.find_value_deltas(meta[:building]), :tax_values)
  ...
  test "function find_deltas returns Building with both Value and Annual Deltas for each year", meta do
    %{2012 => %TaxValue{value_delta: vd_2012, annual_delta: ad_2012}, 
      ...
      2014 => %TaxValue{value_delta: vd_2014, annual_delta: ad_2014}} = Map.get(Building.DeltaFuns.find_deltas(meta[:building]), :tax_values)
  ...
  test "function find_annual_deltas returns Building with Value Delta when given a Building with one Tax Year" do
    ...
    } |> Building.DeltaFuns.find_annual_deltas
